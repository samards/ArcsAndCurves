class BezelText {
    ...
    function draw(dc, text, angle, font) {
        ...
        while(text != null && t >= 0 && t < text.length()) {
            ...
            if(!char.equals(" ")) { // can skip drawing a space
                dc.drawText(x + 5,y-fh+5,f,char,Graphics.TEXT_JUSTIFY_CENTER);
            }
    ...
    function getTextWidth(iconWidth, text, font) {
        var t = 0;
        var textWidth = iconWidth;
        while(text != null && t < text.length()) {
            var char = text.substring(t, t+1);
            textWidth += font.letterwidths.get(char);
            t++;
        }
        return textWidth;
    }  
